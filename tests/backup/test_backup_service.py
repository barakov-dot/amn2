...
def _update_config_share_token_field(path, field_name, value):
    ...
    conn = connect(path)
    if field_name == "owner_user_id" and value == 0:
        conn.execute(
            """
...
def _drop_config_share_owner_foreign_key_declaration(path):
    conn = sqlite3.connect(path)
    try:
        conn.execute("PRAGMA foreign_keys = OFF")
        conn.executescript(
...
def _drop_config_share_unique_constraint_declarations(path):
    conn = sqlite3.connect(path)
    try:
        conn.execute("PRAGMA foreign_keys = OFF")
        conn.executescript(
...
def _drop_config_share_required_column_declaration(path):
    conn = sqlite3.connect(path)
    try:
        conn.execute("PRAGMA foreign_keys = OFF")
        conn.executescript(
...
def _weaken_config_share_index_declaration_shape(path):
    conn = sqlite3.connect(path)
    try:
        conn.execute("PRAGMA foreign_keys = OFF")
        conn.executescript(
            ...
            SELECT
                ...
                created_at
            FROM config_share_tokens_old;
            CREATE UNIQUE INDEX config_share_tokens_token_hash_config_share_partial_unique
                ON config_share_tokens (token_hash)
...
def _weaken_config_share_column_declaration_shape(path):
    conn = sqlite3.connect(path)
    try:
        conn.execute("PRAGMA foreign_keys = OFF")
        conn.executescript(
...
@pytest.mark.parametrize(
    ...
    [
        ("id", ""),
        ("token_hash", "not-a-sha256-hash"),
        ("token_prefix", ""),
        ("created_by_actor", ""),
        ("owner_user_id", 0),
    ],
)
def test_backup_create_rejects_malformed_config_share_token_identity_metadata(
    tmp_path,
    monkeypatch,
    field_name,
    field_value,
    ...
    with pytest.raises(ValueError, match="config share token.*identity metadata"):
        service.create(db_path=db_path, output_dir=tmp_path / "backups")
...
def test_backup_create_rejects_malformed_config_share_token_expires_at_even_when_revoked(
    ...
    with pytest.raises(ValueError, match="config share token.*expires_at"):
        service.create(db_path=db_path, output_dir=tmp_path / "backups")
...
def test_backup_create_rejects_missing_config_share_owner_foreign_key_declaration(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("APP_SECRET_KEY", STRONG_SECRET)
    db_path = tmp_path / "source.sqlite3"
    _create_database_with_config_share_token_history(db_path, "expired")
    _drop_config_share_owner_foreign_key_declaration(db_path)
...
def test_backup_create_rejects_missing_config_share_unique_constraint_declarations(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("APP_SECRET_KEY", STRONG_SECRET)
    db_path = tmp_path / "source.sqlite3"
    _create_database_with_config_share_token_history(db_path, "expired")
    _drop_config_share_unique_constraint_declarations(db_path)

    service = BackupService(app_version="0.1.0")

    with pytest.raises(ValueError, match="unique constraint schema"):
        service.create(db_path=db_path, output_dir=tmp_path / "backups")


def test_backup_create_rejects_missing_config_share_required_column_declarations(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("APP_SECRET_KEY", STRONG_SECRET)
    db_path = tmp_path / "source.sqlite3"
    _create_database_with_config_share_token_history(db_path, "expired")
    _drop_config_share_required_column_declaration(db_path)
...
def test_backup_create_rejects_weakened_config_share_column_declaration_shape(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("APP_SECRET_KEY", STRONG_SECRET)
    db_path = tmp_path / "source.sqlite3"
    _create_database_with_config_share_token_history(db_path, "expired")
    _weaken_config_share_column_declaration_shape(db_path)

    service = BackupService(app_version="0.1.0")

    with pytest.raises(ValueError, match="column declaration schema"):
        service.create(db_path=db_path, output_dir=tmp_path / "backups")


def test_backup_create_rejects_weakened_config_share_index_declaration_shape(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("APP_SECRET_KEY", STRONG_SECRET)
    db_path = tmp_path / "source.sqlite3"
    _create_database_with_config_share_token_history(db_path, "expired")
    _weaken_config_share_index_declaration_shape(db_path)

    service = BackupService(app_version="0.1.0")

    with pytest.raises(ValueError, match="index declaration schema"):
        service.create(db_path=db_path, output_dir=tmp_path / "backups")
...
def test_restore_rejects_malformed_config_share_token_identity_metadata_before_writing_target(
    tmp_path,
    monkeypatch,
    field_name,
    field_value,
    ...
    backup_path = _write_encrypted_archive(
        tmp_path / "malformed-share-token-identity-backup.tar.enc",
        [
            _regular_member("database.sqlite3", database_payload),
            _regular_member("manifest.json", json.dumps(manifest).encode("utf-8")),
        ],
...
def test_restore_rejects_malformed_config_share_token_revoked_at_before_writing_target(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("APP_SECRET_KEY", STRONG_SECRET)
    ...
    backup_path = _write_encrypted_archive(
        tmp_path / "malformed-share-token-timestamp-backup.tar.enc",
        [
            _regular_member("database.sqlite3", database_payload),
            _regular_member("manifest.json", json.dumps(manifest).encode("utf-8")),
        ],
...
def test_restore_rejects_missing_config_share_owner_foreign_key_declaration_before_writing_target(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("APP_SECRET_KEY", STRONG_SECRET)
    ...
    backup_path = _write_encrypted_archive(
        tmp_path / "missing-foreign-key-schema-backup.tar.enc",
        [
            _regular_member("database.sqlite3", database_payload),
            _regular_member("manifest.json", json.dumps(manifest).encode("utf-8")),
        ],
...
def test_restore_rejects_missing_config_share_unique_constraint_declarations_before_writing_target(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("APP_SECRET_KEY", STRONG_SECRET)
    ...
    backup_path = _write_encrypted_archive(
        tmp_path / "missing-unique-schema-backup.tar.enc",
        [
            _regular_member("database.sqlite3", database_payload),
            _regular_member("manifest.json", json.dumps(manifest).encode("utf-8")),
        ],
...
def test_restore_rejects_missing_config_share_required_column_declarations_before_writing_target(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("APP_SECRET_KEY", STRONG_SECRET)
    ...
    backup_path = _write_encrypted_archive(
        tmp_path / "missing-required-column-schema-backup.tar.enc",
        [
            _regular_member("database.sqlite3", database_payload),
            _regular_member("manifest.json", json.dumps(manifest).encode("utf-8")),
        ],
...
def test_restore_rejects_weakened_config_share_column_declaration_shape_before_writing_target(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("APP_SECRET_KEY", STRONG_SECRET)
    ...
    backup_path = _write_encrypted_archive(
        tmp_path / "weakened-column-shape-backup.tar.enc",
        [
            _regular_member("database.sqlite3", database_payload),
            _regular_member("manifest.json", json.dumps(manifest).encode("utf-8")),
        ],
...
def test_restore_rejects_weakened_config_share_index_declaration_shape_before_writing_target(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("APP_SECRET_KEY", STRONG_SECRET)
    ...
    backup_path = _write_encrypted_archive(
        tmp_path / "weakened-index-shape-backup.tar.enc",
        [
            _regular_member("database.sqlite3", database_payload),
            _regular_member("manifest.json", json.dumps(manifest).encode("utf-8")),
        ],
