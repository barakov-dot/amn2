...
- `name`;
- `duration_days`;
- `max_devices` - опциональная квота активных клиентских устройств; фактический
  предел равен меньшему из этого значения и `MAX_DEVICES_PER_USER`;
- `price`;
- `currency`;
- `is_free`;
- `is_active`;
...
Web admin страница `/plans` позволяет оператору изменить только
`max_devices`, не затрагивая срок, цену и активность тарифа. Пустое значение
сохраняется как `NULL` и возвращает тариф к глобальному пределу. Каждое
изменение записывается в `admin_actions` с предыдущим, новым и фактическим
значением квоты.
