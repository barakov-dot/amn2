...
В web admin раздел `Тарифы` (`/plans`) показывает глобальный предел, квоту
каждого тарифа и фактический лимит. Положительное целое число задаёт
`plans.max_devices`; пустое поле очищает квоту и возвращает глобальный fallback.
Изменение влияет на последующие одобрения новых dedicated-device заявок и
фиксируется действием `web_plan_device_quota_update` в `admin_actions`.
